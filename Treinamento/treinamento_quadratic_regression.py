# -*- coding: utf-8 -*-
"""treinamento_quadratic_regression.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1w3D3_Y8gldLdEll0IAKq6hCuFD0TMWQS
"""
...
X_notebook_poly2 = poly2.fit_transform(X_notebook)
model_distance_notebook = LinearRegression()
model_distance_notebook.fit(X_notebook_poly2, y_notebook)

y_notebook_pred_2 = model_distance_notebook.predict(X_notebook_poly2)

XX = np.arange(-30.0, -80.0, -1.0)

yy = model_distance_notebook.intercept_[0] + model_distance_notebook.coef_[0][1]*XX + model_distance_notebook.coef_[0][2]*(XX**2)

# metricas
print("Mean absolute error: %.2f" % np.mean(np.absolute(y_notebook - y_notebook_pred_2)))
print("Residual sum of squares: %.2f" % np.mean((y_notebook - y_notebook_pred_2)**2))
print("R2-score: %.2f" % r2_score(y_notebook, y_notebook_pred_2))

#%%

plt.plot(y_notebook, X_notebook, 'ro', yy, XX)
...
X_router_poly2 = poly2.fit_transform(X_router)

model_distance_router = LinearRegression()
model_distance_router.fit(X_router_poly2,y_router)

y_router_pred_2 = model_distance_router.predict(X_router_poly2)

XX = np.arange(-30.0, -70.0, -1.0)

yy = model_distance_router.intercept_[0] + model_distance_router.coef_[0][1]*XX + model_distance_router.coef_[0][2]*(XX**2)

# metricas
print("Mean absolute error: %.2f" % np.mean(np.absolute(y_router - y_router_pred_2)))
print("Residual sum of squares: %.2f" % np.mean((y_router - y_router_pred_2)**2))
print("R2-score: %.2f" % r2_score(y_router, y_router_pred_2))

#%%

plt.plot(y_router, X_router, 'ro', yy, XX)
plt.xlabel('Distance (m)')
plt.ylabel('RSSI (dBm)')
...
X_repeater_poly2 = poly2.fit_transform(X_repeater)

model_distance_repeater = LinearRegression()
model_distance_repeater.fit(X_repeater_poly2, y_repeater)

y_repeater_pred_2 = model_distance_repeater.predict(X_repeater_poly2)


XX = np.arange(-25.0, -45.0, -1.0)

yy = model_distance_repeater.intercept_[0] + model_distance_repeater.coef_[0][1]*XX + model_distance_repeater.coef_[0][2]*(XX**2)

# metricas
print("Mean absolute error: %.2f" % np.mean(np.absolute(y_repeater - y_repeater_pred_2)))
print("Residual sum of squares: %.2f" % np.mean((y_repeater - y_repeater_pred_2)**2))
print("R2-score: %.2f" % r2_score(y_repeater, y_repeater_pred_2))

#%%

plt.plot(y_repeater, X_repeater, 'ro', yy, XX)
plt.xlabel('Distance (m)')
plt.ylabel('RSSI (dBm)')
...
filename = "distance_quadratic_regression.sav"
