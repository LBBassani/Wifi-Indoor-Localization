# -*- coding: utf-8 -*-
"""treinamento_cubic_regression.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1tsk1ujSgWC9fuzT6VbJ4GOUPGT_02COo
"""
...
model_distance_notebook.fit(X_notebook_poly3, y_notebook)

y_notebook_pred_3 = model_distance_notebook.predict(X_notebook_poly3)

XX = np.arange(-30.0, -80.0, -1.0)

yy = model_distance_notebook.intercept_[0] + model_distance_notebook.coef_[0][1]*XX + model_distance_notebook.coef_[0][2]*(XX**2) + model_distance_notebook.coef_[0][3]*(XX**3)

# metricas
print("Mean absolute error: %.2f" % np.mean(np.absolute(y_notebook - y_notebook_pred_3)))
print("Residual sum of squares: %.2f" % np.mean((y_notebook - y_notebook_pred_3)**2))
print("R2-score: %.2f" % r2_score(y_notebook, y_notebook_pred_3))
...
model_distance_router.fit(X_router_poly3,y_router)

y_router_pred_3 = model_distance_router.predict(X_router_poly3)

XX = np.arange(-30.0, -70.0, -1.0)

yy = model_distance_router.intercept_[0] + model_distance_router.coef_[0][1]*XX + model_distance_router.coef_[0][2]*(XX**2) + model_distance_router.coef_[0][3]*(XX**3)

# metricas
print("Mean absolute error: %.2f" % np.mean(np.absolute(y_router - y_router_pred_3)))
print("Residual sum of squares: %.2f" % np.mean((y_router - y_router_pred_3)**2))
print("R2-score: %.2f" % r2_score(y_router, y_router_pred_3))
...
model_distance_repeater.fit(X_repeater_poly3, y_repeater)

y_repeater_pred_3 = model_distance_repeater.predict(X_repeater_poly3)

XX = np.arange(-25.0, -45.0, -1.0)

yy = model_distance_repeater.intercept_[0] + model_distance_repeater.coef_[0][1]*XX + model_distance_repeater.coef_[0][2]*(XX**2) + model_distance_repeater.coef_[0][3]*(XX**3)

# metricas
print("Mean absolute error: %.2f" % np.mean(np.absolute(y_repeater - y_repeater_pred_3)))
print("Residual sum of squares: %.2f" % np.mean((y_repeater - y_repeater_pred_3)**2))
print("R2-score: %.2f" % r2_score(y_repeater, y_repeater_pred_3))
...
filename = "distance_cubic_regression.sav"
